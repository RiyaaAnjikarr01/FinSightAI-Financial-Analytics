...
con = duckdb.connect(os.path.join(os.path.dirname(__file__), "..", "..", "finance_analytics_light.duckdb"))
...
WITH base AS (
    SELECT
        DATE_TRUNC('month', DATE(STRPTIME(TransactionDate, '%d.%m.%Y'))) AS TransactionMonth,
        StoreCode,
        AccountNumber,
